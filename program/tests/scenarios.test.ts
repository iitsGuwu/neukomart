...
(async () => {
  ...
  await test('update_listing changes price → buy clears at the NEW price', async () => {
    ...
    await retry(() => program.methods.purchaseWithSol(newP).accountsPartial({ buyer: B.publicKey, seller: A.publicKey, listing, asset: a, collection: HARMIES, mplCoreProgram: MPL_CORE, systemProgram: SYS }).signers([B]).rpc(), 'buy');
  ...
  await test('update_listing changes currency SOL→$GBOY → $GBOY purchase works', async () => {
    ...
    await retry(() => program.methods.purchaseWithGboy(price).accountsPartial({ buyer: B.publicKey, seller: A.publicKey, listing, gboyMint: GBOY, buyerGboy: wAtaB, sellerGboy: wAtaA, asset: a, collection: HARMIES, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'buy-gboy');
    assert((await ownerOf(a)) === B.publicKey.toBase58(), 'asset to buyer');
  ...
  await test('floor offer (SOL): any matching asset can be sold into the bid', async () => {
    ...
    const before = await solBal(A.publicKey);
    await retry(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: a, collection: HARMIES, offerGboy: null, sellerGboy: null, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), 'accept-floor');
  ...
  await test('accept_offer on a specific-asset bid rejects the wrong asset (AssetMismatch)', async () => {
    ...
    await expectRevert(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: sa('HA2'), collection: HARMIES, offerGboy: null, sellerGboy: null, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), /AssetMismatch/, 'wrong-asset');
  ...
  await test('accept_offer rejects an asset the seller does not own (NotAssetOwner)', async () => {
    ...
    await expectRevert(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: sa('HB1'), collection: HARMIES, offerGboy: null, sellerGboy: null, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), /NotAssetOwner/, 'not-owned');
    await cancelOfferIfExists(n);
  });
