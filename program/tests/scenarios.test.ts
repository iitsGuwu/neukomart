...
async function cancelSwapIfExists(nonce: BN, offered: PublicKey[], coll: PublicKey, gboy = false) {
  ...
  await retry(() => program.methods.cancelSwap()
    .accountsPartial({ maker: A.publicKey, swapOffer: swap, makerGboy: gboy ? wAtaA : undefined, swapGboy: gboy ? ataOf(swap) : undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
...
async function cancelOfferIfExists(nonce: BN, gboy = false) {
  ...
  await retry(() => program.methods.cancelOffer()
    .accountsPartial({ bidder: B.publicKey, offer, bidderGboy: gboy ? wAtaB : undefined, offerGboy: gboy ? ataOf(offer) : undefined, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
    .signers([B]).rpc(), 'reclaim-offer');
}
const swapArgs = (o: Partial<any>) => ({ offeredCount: 0, requestedAssets: [], solOffered: new BN(0), gboyOffered: new BN(0), solRequested: new BN(0), gboyRequested: new BN(0), taker: undefined, ...o });
...
(async () => {
  ...
  await test('update_listing changes currency SOL→$GBOY → $GBOY purchase works', async () => {
    ...
    assert((await ownerOf(a)) === B.publicKey.toBase58(), 'asset to buyer');
    assert((await gboyBal(wAtaA)) - before === BigInt(price.toString()), 'seller paid in $GBOY');
  });
  ...
  async function runSwap(label: string, nonce: BN, opts: { offered: string[]; requested: string[]; offColl?: PublicKey; reqColl?: PublicKey; solOffered?: number; gboyOffered?: number; solRequested?: number; gboyRequested?: number; taker?: PublicKey | undefined }) {
  ...
  await test('swap 2↔2 NFTs settles all four owners atomically', async () => {
    ...
    await retry(() => program.methods.acceptSwap().accountsPartial({ taker: B.publicKey, maker: A.publicKey, swapOffer: s.swap, takerGboy: undefined, makerGboy: undefined, swapGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
  ...
  await test('swap with $GBOY top-up REQUESTED → taker pays $GBOY to maker (legit redirect path)', async () => {
    ...
    const before = await gboyBal(wAtaA);
    await retry(() => program.methods.acceptSwap().accountsPartial({ taker: B.publicKey, maker: A.publicKey, swapOffer: s.swap, takerGboy: wAtaB, makerGboy: wAtaA, swapGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
      .remainingAccounts([ra(s.requested[0], true), ra(HARMIES, false), ra(s.offered[0], true), ra(HARMIES, false)]).signers([B]).rpc(), 'accept');
    assert((await ownerOf(s.offered[0])) === B.publicKey.toBase58(), 'NFT → taker');
  ...
  await test('cancel_swap (SOL top-up) → maker reclaims the NFT and the SOL', async () => {
    ...
    await retry(() => program.methods.cancelSwap().accountsPartial({ maker: A.publicKey, swapOffer: s.swap, makerGboy: undefined, swapGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
  ...
  await test('create_swap with nothing offered is rejected (EmptyOffer)', async () => {
    ...
    await expectRevert(() => program.methods.createSwap(n, swapArgs({ offeredCount: 0, requestedAssets: [sa('HB1')] }))
      .accountsPartial({ maker: A.publicKey, swapOffer: swapPda(A.publicKey, n), makerGboy: undefined, swapGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS })
  ...
  await test('accept_swap rejects a wrong delivered asset (AssetMismatch)', async () => {
    ...
    await expectRevert(() => program.methods.acceptSwap().accountsPartial({ taker: B.publicKey, maker: A.publicKey, swapOffer: s.swap, takerGboy: undefined, makerGboy: undefined, swapGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS })
  ...
  await test('floor offer (SOL): any matching asset can be sold into the bid', async () => {
    ...
    await retry(() => program.methods.createOffer(n, { collection: HARMIES, asset: undefined, amount: new BN(0.01 * LAMPORTS), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer, bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'create-floor');
    const before = await solBal(A.publicKey);
    await retry(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: a, collection: HARMIES, creator: creatorFor(HARMIES), creatorGboy: undefined, offerGboy: undefined, sellerGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), 'accept-floor');
  ...
  await test('create_offer with amount 0 is rejected (ZeroPrice)', async () => {
    ...
    await expectRevert(() => program.methods.createOffer(n, { collection: HARMIES, asset: undefined, amount: new BN(0), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer: offerPda(B.publicKey, n), bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), /ZeroPrice/, 'zero-amount');
  });
  ...
  await test('create_offer on a foreign collection is rejected (CollectionNotAllowed)', async () => {
    ...
    await expectRevert(() => program.methods.createOffer(n, { collection: FOREIGN, asset: undefined, amount: new BN(0.01 * LAMPORTS), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer: offerPda(B.publicKey, n), bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), /CollectionNotAllowed/, 'foreign-offer');
  });
  ...
  await test('accept_offer on a specific-asset bid rejects the wrong asset (AssetMismatch)', async () => {
    ...
    await retry(() => program.methods.createOffer(n, { collection: HARMIES, asset: target, amount: new BN(0.01 * LAMPORTS), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer, bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'create-specific');
    await expectRevert(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: sa('HA2'), collection: HARMIES, creator: creatorFor(HARMIES), creatorGboy: undefined, offerGboy: undefined, sellerGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), /AssetMismatch/, 'wrong-asset');
  ...
  await test('accept_offer rejects an asset the seller does not own (NotAssetOwner)', async () => {
    ...
    const offer = offerPda(B.publicKey, n);
    await retry(() => program.methods.createOffer(n, { collection: HARMIES, asset: undefined, amount: new BN(0.01 * LAMPORTS), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer, bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'create-floor2');
    await expectRevert(() => program.methods.acceptOffer().accountsPartial({ seller: A.publicKey, bidder: B.publicKey, offer, asset: sa('HB1'), collection: HARMIES, creator: creatorFor(HARMIES), creatorGboy: undefined, offerGboy: undefined, sellerGboy: undefined, mplCoreProgram: MPL_CORE, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([A]).rpc(), /NotAssetOwner/, 'not-owned');
    await cancelOfferIfExists(n);
  });
  ...
  await test('cancel_offer (SOL) returns the full escrow to the bidder', async () => {
    ...
    const offer = offerPda(B.publicKey, n);
    const before = await solBal(B.publicKey);
    await retry(() => program.methods.createOffer(n, { collection: HARMIES, asset: undefined, amount: new BN(0.02 * LAMPORTS), currency: { sol: {} } })
      .accountsPartial({ bidder: B.publicKey, offer, bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, associatedTokenProgram: ASSOCIATED_TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'create');
    await retry(() => program.methods.cancelOffer().accountsPartial({ bidder: B.publicKey, offer, bidderGboy: undefined, offerGboy: undefined, tokenProgram: TOKEN_PROGRAM_ID, systemProgram: SYS }).signers([B]).rpc(), 'cancel');
  ...
  await test('purchase_with_sol rejects a $GBOY listing (WrongCurrency)', async () => {
    ...
    await expectRevert(() => program.methods.purchaseWithSol(new BN(LAMPORTS)).accountsPartial({ buyer: B.publicKey, seller: A.publicKey, listing, asset: a, collection: HARMIES, mplCoreProgram: MPL_CORE, systemProgram: SYS }).signers([B]).rpc(), /WrongCurrency/, 'wrong-cur-sol');
    await clearListing(a);
  });
